// Cloudflare Pages Functions - Google Calendar connector OAuth
// Menukar authorization code Google (scope Calendar) jadi access + refresh token.
...
export async function onRequestPost({ request, env }) {
  try {
    ...
    if (!user) {
      return jsonResponse({ error: 'Silakan login Clincoo terlebih dahulu untuk menghubungkan Google Calendar.' }, 401);
    }
    ...
    if (body.refresh_token && !body.code) {
      ...
      if (!tokenData.access_token) {
        return jsonResponse({ error: (tokenData.error_description || tokenData.error) || 'Gagal memperbarui token Calendar.' }, 401);
      }
      return jsonResponse({
    ...
    if (!tokenData.access_token) {
      ...
      return jsonResponse({ error: 'Gagal menghubungkan Google Calendar: ' + reason }, 400);
    }
